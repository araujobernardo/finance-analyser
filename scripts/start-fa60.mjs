...
const transRes = await fetch(`${BASE_URL}/rest/api/3/issue/FA-60/transitions`, { headers: HEADERS });
const transData = await transRes.json();
const transition = transData.transitions.find(t => t.name === "In Progress");
if (!transition) {
  console.error("Could not find 'In Progress' transition. Available:", transData.transitions.map(t => t.name));
  process.exit(1);
}

// Move to In Progress
const moveRes = await fetch(`${BASE_URL}/rest/api/3/issue/FA-60/transitions`, {
...
// Add comment
const commentRes = await fetch(`${BASE_URL}/rest/api/3/issue/FA-60/comment`, {
  method: "POST", headers: HEADERS,
  body: JSON.stringify({
    body: {
      type: "doc", version: 1,
